...
const ENDPOINT = "https://calm-plains-76927.herokuapp.com";
...
const MsgBox = ({ seletedChat, chatUser }) => {
  ...
  const msgRecieved = () => {
    socket.on("message recieved", (newMessageRecieved) => {
      if (
        !selectetChatCompare ||
        selectetChatCompare !== newMessageRecieved.chat._id
      ) {
        console.log("he");
      } else {
  ...
  const sendMessage = async () => {
    ...
    try {
      const { data } = await axios.post(
        `https://calm-plains-76927.herokuapp.com/message`,
        info
      );
      // console.log(data)
      setnewMessage("");
  ...
  const getallmessage = async () => {
    ...
    try {
      const { data } = await axios.get(
        `https://calm-plains-76927.herokuapp.com/message/${seletedChat}`
      );
      setallMessage(data);
      setisloding(false);
      ...
      {seletedChat ? (
        <div>
          <div className="msg-container">
            ...
            {isTypeing ? (
              <div className="typing-div text-center">
                <Lottie options={defaultOptions} height={30}></Lottie>
              </div>
            ) : (
              <></>
            )}
