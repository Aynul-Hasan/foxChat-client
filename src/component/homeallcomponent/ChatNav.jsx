...
const ChatNav = ({ event, seletedChat, chatUser }) => {
  ...
  const changeGroupName = async () => {
    ...
    try {
      ...
      const { data } = await axios.put(
        `https://calm-plains-76927.herokuapp.com/chat/rename`,
        info
      );
      setgroupName("");
      callChats(chatUser);
  ...
  const AddMemberInGroup = async (_id) => {
    try {
      ...
      const { data } = await axios.put(
        `https://calm-plains-76927.herokuapp.com/chat/addmember`,
        info
      );
      setselectUsers([]);
      callChats(chatUser);
  ...
  const removeMemberIntoGroup = async (_id) => {
    try {
      ...
      const { data } = await axios.put(
        `https://calm-plains-76927.herokuapp.com/chat/removemember`,
        info
      );
      callChats(chatUser);
      if (data) {
    ...
    <>
      <div className="container d-flex align-items-center justify-content-between mt-4">
        <div className="d-flex align-items-center">
          ...
          <div className="ms-3">
            <strong>
              {user[0]?.isGroupChat ? user[0]?.chatName : curChatBox?.name}
            </strong>
          </div>
        ...
        {user[0]?.isGroupChat ? (
          <div>
            ...
            >
              ...
              <img
                width=""
                src="https://img.icons8.com/external-anggara-flat-anggara-putra/32/000000/external-Setting-user-interface-anggara-flat-anggara-putra-2.png"
                alt=""
              />
            </button>
            {/* modal */}
              ...
              <div className="modal-dialog">
                <div className="modal-content">
                  ...
                  <div className="modal-body">
                    ...
                    <div className="d-flex align-items-center">
                      {user[0].users.map((u) => (
                        <div key={u._id} className=" p-1 m-1 bg ">
                          <img src={u.image} width="30" alt="" />
                          {user[0]?.groupAdmin.email === curUser.email ? (
                            <button
                              onClick={() => removeMemberIntoGroup(u._id)}
                              type="button"
                    ...
                    {user[0]?.groupAdmin.email === curUser.email ? (
                      <div className="mt-3 d-flex">
                        ...
                        <button
                          className="btn bg text-white"
                          onClick={() => AddMemberInGroup(selectUsers[0]._id)}
                        >
                          Add
                        </button>
                      </div>
                  ...
                  <div className="modal-footer">
                    <button
                      type="button"
                      onClick={() => removeMemberIntoGroup(chatUser)}
