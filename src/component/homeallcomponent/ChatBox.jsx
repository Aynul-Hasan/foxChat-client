...
const ChatBox = ({event,chatuser,userData,click,setseletedChat}) => {
  ...
  useEffect(() => {
    const getData=async()=>{
        setdataLoding(true)
        const {data}= await axios.get(`http://localhost:8000/chat/${userData?._id}`)
        // console.log(data)
        setchats(data)
  ...
  return (
    <>
      {/* {dataLoding?<div className="text-center"><ScaleLoader color={color}/></div>:""} */}
       {chats.map((chat)=><ChatList key={chat._id} curuserId={userData?._id}  chat={chat} event={event} setseletedChat={setseletedChat} />)  }
       <div className="">
...
const ChatList=({event,chat,setseletedChat,curuserId})=>{
  ...
  return(
    <>
       <div onClick={()=>setData(chat?._id,id[0])} className="container chat py-2 d-flex mt-1 align-items-center">
        ...
        <div className="chat-contain">
          ...
          <br />
          <p>{curuserId===chat?.latestMessage?.sender?._id?"You":chat?.latestMessage?.sender?.name}:{chat?.latestMessage?.content}</p>
        </div>
